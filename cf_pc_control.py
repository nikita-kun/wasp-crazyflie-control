...
#        yawrate_r = 100.0
        ...
        # Second controller
#        plane_rotation = np.array([[np.cos(yaw), -np.sin(yaw)],
#                                   [np.sin(yaw), np.cos(yaw)]])
#        plane_control_signal = np.array([[self.pid_pos_kp * ex + self.pid_pos_kd * exrate],
#                                         [-(self.pid_pos_kp * ey + self.pid_pos_kd * eyrate)]])
#        thrust_const = self.pid_C / (np.cos(roll) * np.cos(pitch))
#        roll_r, pitch_r = np.matmul(plane_rotation, plane_control_signal)
#        yawrate_r = self.pid_yaw_kd * yawrate
#        thrust_r = thrust_const * (self.pid_alt_kp * ez + self.pid_alt_kd * ezrate + self.pid_mg)
